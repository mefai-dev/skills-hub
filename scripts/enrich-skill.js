...
async function enrich(submissionFilePath) {
  ...
  if (process.env.AGENTGUARD_API_KEY) {
    ...
    if (!skillContent) {
      console.warn('  Â·š  No scannable content found in repo Â·Â·Â· skipping AgentGuard');
    } else {
      ...
      try {
        ...
        if (agRes.ok) {
          ...
          console.warn(`  Â·š  AgentGuard returned ${agRes.status}: ${errBody} Â·Â·Â· skipping`);
        }
      } catch (err) {
        console.warn(`  Â·š  AgentGuard call failed: ${err.message} Â·Â·Â· skipping`);
      }
    }
  } else {
    console.warn('  Â·š  AGENTGUARD_API_KEY not set Â·Â·Â· skipping security scan');
  }
...
(async () => {
  ...
  for (const filePath of args) {
    try {
      ...
      console.error(`Â·œ— Failed [${filePath}]: ${err.message}`);
      hasError = true;
    }
  }
