...
Submitting a skill takes about 2 minutes. You create one JSON file 路路路 the rest is automated.
...
|-------|------|----------|-------|
| `name` | string | No | Display name 路路路 defaults to filename if omitted |
...
> Example: `my-skill-metadata.json` 路啋 identifier `my-skill`
...
- **Public repos only** 路路路 private repos cannot be validated
- **No duplicates** 路路路 check the `skills/` folder before submitting
- **One PR per skill** 路路路 keeps review focused
